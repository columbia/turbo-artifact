...
class Task:
    def __init__(self, id, num_blocks, type):
        ...
        for i in range(num_blocks):
            self.budget_per_block[i] = curves.ZeroCurve().budget
...
def create_laplace_task(task_id, num_blocks, block_ids, noise):
    ...
    for block_id in block_ids:
        task.budget_per_block[block_id] = curves.LaplaceCurve(laplace_noise=noise).budget
    return task
...
def create_gaussian_task(task_id, num_blocks, block_ids, sigma):
    ...
    for block_id in block_ids:
        task.budget_per_block[block_id] = curves.GaussianCurve(sigma=sigma).budget
    return task
...
def create_subsamplegaussian_task(task_id, num_blocks, block_ids, ds, bs, epochs, s):
    ...
    for block_id in block_ids:
        task.budget_per_block[block_id] = curves.SubsampledGaussianCurve.from_training_parameters(ds, bs, epochs,
                                                                                                  s).budget
    return task
