...
def analyze_monoblock(experiment_path):
    ...
    def plot_budget_utilization(df, total_tasks):
        ...
        fig.write_image(
            LOGS_PATH.joinpath(f"{experiment_path}/budget_utilization.png")
        )
        return fig
    ...
    def plot_cumulative_budget_utilization_time(df, total_tasks):
        df.to_csv(
            LOGS_PATH.joinpath(
                f"{experiment_path}/cumulative_budget_utilization.csv"
            ),
            index=False,
        )
        fig = px.line(
        ...
        fig.write_image(
            LOGS_PATH.joinpath(
                f"{experiment_path}/cumulative_budget_utilization.png"
            )
        )
        return fig
...
def analyze_multiblock(experiment_path):
    ...
    def plot_budget_utilization(df, total_tasks):
        ...
        for key, group in grouped:
            ...
            pivoted_df.to_csv(
                LOGS_PATH.joinpath(f"{experiment_path}/budget_utilization_zipf_{key}.csv"),
                index=False,
            )
