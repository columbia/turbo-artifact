...
class PMW:
    ...
    def run(self, query, true_output):
        ...
        if noisy_error < self.noisy_threshold:
            ...
            update_tensor = torch.mul(self.histogram.tensor, torch.exp(query_tensor_dense * lr))
            bins_mask = query_tensor_dense == 0
            self.histogram.tensor = torch.add(torch.mul(self.histogram.tensor, bins_mask), update_tensor)
